          ...
          <div className="min-h-screen py-10 bg-white">
               <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                         {tilesData.map(tile => (
                              <div key={tile.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-md hover:shadow-lg transition-shadow duration-300 w-full">
                                   <div className="flex flex-col xl:flex-row gap-5 items-start">
                                        ...
                                        <div className="flex-1 min-w-0">
                                             ...
                                             <p className="text-sm text-gray-600 mb-2 leading-snug">{tile.description}</p>
                                             <div className="space-y-1 text-sm text-gray-800">
                                                  <div><span className="font-semibold">Series:</span> {tile.series}</div>
                                                  <div><span className="font-semibold">Category:</span> {tile.category}</div>
                                                  <div><span className="font-semibold">Suitable Place:</span> {tile.suitablePlace}</div>
                                                  <div><span className="font-semibold">Size:</span> {tile.size}</div>
                                             </div>
                                        </div>
                                   </div>
                              </div>
