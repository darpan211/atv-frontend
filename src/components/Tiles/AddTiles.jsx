...
const AddTiles = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema,
    onSubmit: values => {
      // Do nothing (preview button removed)
    },
  });
  ...
  const DropdownWithAdd = ({ label, options, selectedValues, onChange }) => (
    <div className="w-full">
      ...
      <div className="flex gap-1 items-center  ">
        ...
        <button
          type="button"
          className="h-10 w-10 rounded-md bg-[#7b4f28] text-white flex items-center justify-center hover:bg-[#633e1f]"
        >
          +
        </button>
      </div>
      ...
      <div className="bg-[#FFF5EE] max-w-7xl mx-auto p-6 rounded-xl shadow-md border border-gray-200">
        <form onSubmit={formik.handleSubmit}>
          <div className="grid items-center grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <DropdownWithAdd
              label="Size"
              options={sizes?.list?.map(item => ({ label: item?.sizes, value: item?.id })) || []}
            ...
            <DropdownWithAdd
              label="Series"
              options={series?.list?.map(item => ({ label: item?.series, value: item?.id })) || []}
            ...
            <DropdownWithAdd
              label="Material"
              options={
                categories?.list?.map(item => ({ label: item?.category, value: item?.id })) || []
              }
