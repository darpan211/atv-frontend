export const DeleteIcon = ({ height, width, size, className,colour='#6F4E37' }) => {
  return (
    <svg
      width={width ?? size ?? 25}
      height={height ?? size ?? 27}
      viewBox="0 0 25 27"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
    >
      <path d="M9.70117 13.5V20.4444" stroke={colour} strokeWidth="2" />
      <path d="M15.2573 13.5V20.4444" stroke={colour} strokeWidth="2" />
      <path d="M1.36816 6.55566H23.5903" stroke={colour} strokeWidth="2" />
      <path
        d="M4.14551 10.7222V21.8333C4.14551 24.1345 6.01099 25.9999 8.31216 25.9999H16.6455C18.9467 25.9999 20.8121 24.1345 20.8121 21.8333V10.7222"
        stroke={colour}
        strokeWidth="2"
      />
      <path
        d="M8.31299 3.77777C8.31299 2.24365 9.55664 1 11.0908 1H13.8685C15.4027 1 16.6463 2.24365 16.6463 3.77777V6.55554H8.31299V3.77777Z"
        stroke={colour}
        strokeWidth="2"
      />
    </svg>
  );
};
